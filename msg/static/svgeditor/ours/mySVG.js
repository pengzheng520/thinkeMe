//2016.5.3. 修改在点上提示智能作图
//2016.5.4 修改垂直相等时点的位置不精确的问题L3410,问题：toFixed()返回的是字符串，不是数字
//2016.5.7 移动直线问题————————————————————————未处理
//2016.5.7 修改因增大点而引起的点选定问题
//2016.5.7 修改大于180度的角度标注问题
//2016.5.8 新增点在线段上的逻辑动画
...
*/
	if (true)//this.editable)
	{
		...
		this.width=(typeof width == "undefined" || width==null)?(parseInt($("#"+parentID).width())):width;
...
//this.origin=[0,0];
	...
	//varA["varName"]={"val":10,"min":1,"max:":100,"step":0.01,"times":100,direction:,"handle":null};
  ...
  // parentDiv.addClass('row'); 
   ...
   if (this.editable)
   {
	...
	var svg_edt_data = 
      ...
      +'<button type="button" class="btn btn-default" id="choice" data-tool="S">'
		+'<img src="../../static/image/tool-select.png" width="14" height="16" border="0" alt="选择" />'
	  +'</button>'
...
SVG.prototype.action=function(x,y){  //存储本地作图指令，判断是否向服务器发送指令
   ...
   if (this.sys && !y)
   {
	   //向服务器发送作图指令
	   var data = {
		   groupID     : $(".group-interface", window.parent.document).attr('data-groupID'),
		   receiver    : $(".group-interface", window.parent.document).attr('data-id'),
		   content     : x
	   };
	   window.parent.firstMsg.socket.emit('svgDirective',data);
   }
}
...
//设置了大小就有问题哈
    ...
    */
	...
	if (true)//this.editable)
	{
		...
		this.width=(typeof width == "undefined" || width==null)?(parseInt($("#"+(this.ID).substr(1)).width())):width;
...
//逻辑动画
SVG.prototype.Animate=function(options){
...
SVG.prototype.moveOnLine=function(pID,lID){//点在线上的逻辑动画
	//var x1,y1,x2,y2;
	var othis=this;
	othis.timer[pID]={"type":1,"step":1,"times":50,"handle":null};
	othis.timer[pID].handle=setInterval(function(){
		...
		var x=parseInt(point.getAttribute("cx"));
		var y=parseInt(point.getAttribute("cy"));
		if (Math.abs(x2-x1)>Math.abs(y2-y1))//以x坐标为准
		{
			if (othis.timer[pID].step>0)
			{
				if (x>Math.max(x1,x2))//还应该考虑type，是来回或是循环
				{
					x=Math.min(x1,x2);
				}
				else{
					x+=othis.timer[pID].step;
				}				
			}
			else{
				if (x<Math.min(x1,x2))//还应该考虑type，是来回或是循环
				{
					x=Math.max(x1,x2);
				}
				else{
			...
			console.log("l_l:"+l_l+"|x,y:"+x+","+y);
			//othis.movePoint(pID,x,y);
			othis.moveObject(pID,x,y);	
		}
		else{
			if (othis.timer[pID].step>0)
			{
				if (y>Math.max(y1,y2))//还应该考虑direction，是来回或是循环
				{
					y=Math.min(y1,y2);
				}
				else{
					y+=othis.timer[pID].step;
				}				
			}
			else{
				if (y<Math.min(y1,y2))//还应该考虑direction，是来回或是循环
				{
					y=Math.max(y1,y2);
				}
				else{
			...
			//othis.movePoint(pID,x,y);
			othis.moveObject(pID,x,y);	
		}},othis.timer[pID].times);

	//}
}


SVG.prototype.onMouseClick=function(e,event){  //mouse click
	...
	{
		//alert(e.target.id);
		var two=e.target.id.substring(0,2);
		var targetID=e.target.id;
		if (allID2.indexOf(two)>0)
		{
			...
			if (this.isInSelected(targetID))
			{
				this.setObjAttr(targetID);
				//删除再次被选择的元素
				for (var i in this.selectedObj){
					if (this.selectedObj[i].id==targetID)
					{
				...
				this.selectedObj.push({"name":getNameById(targetID),"id":targetID});
				//this.getObjAttr(e.target.id);
				//console.log("etarget:"+e.target.id);
				var target=document.getElementById(targetID);
				target.setAttributeNS(null,"stroke","blue");
			...
			case "1":
				...
				switch (s[0].name.substring(0,1))
				{
					case "点":
						...
						//动画
						var pID,tip;
						pID=pID=this.selectedObj[0].id;
						

						//检查点的被限制条件
						var nCon=$("#X"+pID).children().length;
						if (nCon==1){//被限制条件
							var xconID=$("#X"+pID).children().eq(0).text();
							var con=$("#"+xconID).text().split(",");
							//tip="条件:"+xcon;
							switch (con[0])
							{
								case "POL"://点在线上的逻辑动画
									tip="作"+this.selectedObj[0].name+"在"+getNameById(con[2])+"运动的动画";
									break;
								default:
									break;						
...
SVG.prototype.onMouseDown=function(e,event){  //mouse down
  
  //alert("mousedown e.target.id:"+e.target.id);
  
  this.mouseState="down";
  var startx = parseInt(e.pageX-$("#"+this.ID).offset().left);
  var starty = parseInt(e.pageY-$("#"+this.ID).offset().top);
  
  this.startPoint=[startx,starty,null];
  
  //this.myAlert("type:"+typeof this.startPoint[0] +"::"+typeof this.startPoint[1]);
...
//						alert(pre[0])
		...
		case "C":
			 switch (overObjID)
			 {
			    case "BF":
				case "BC"://带背景的点
					this.startPoint=[$("#"+e.target.id.substring(1)).attr("cx"),$("#"+e.target.id.substring(1)).attr("cy"),e.target.id.substring(1)];			      
					break;
				case "FP":
				case "CP":
		...
		case "P":
			...
			switch (overObjID)
			{
				case "BF":
				case "BC"://带背景的点
					this.startPoint=[parseInt($("#"+e.target.id.substring(1)).attr("cx")),parseInt($("#"+e.target.id.substring(1)).attr("cy")),e.target.id.substring(1)];			      
					break;
				case "FP":				   
				case "CP":
					this.startPoint=[parseInt($("#"+e.target.id).attr("cx")),parseInt($("#"+e.target.id).attr("cy")),e.target.id];			      
					break;
				case "CR":  //在圆上作点
				...
				case "PP":
					...
					{						
					...
					else{//else if (this.smartTip.type!="")
					   ...
					   //parseFloat()
						...
						if (Math.abs(x2-x1)>Math.abs(y2-y1))
						{
							//以x坐标为准
							l_l=(startx-x1)/(x2-x1);
							starty=parseInt((l_l*(y2-y1)+y1).toFixed());
							//starty=(l_l*(y2-y1)+y1).toFixed();
						}	
						else{
							l_l=(starty-y1)/(y2-y1);
							startx=parseInt((l_l*(x2-x1)+x1).toFixed());
							//startx=(l_l*(x2-x1)+x1).toFixed();
						}
					   ...
					   */
						...
						this.startPoint=[startx,starty,id];
					   //添加点在线上的约束条件
					   //alert("type:"+typeof this.startPoint[1]);
						var conID="c"+this.newChildID();				   
...
var cznumber=0;
SVG.prototype.onMouseMove=function(e,event){  //mouse move
	...
	//this.myAlert("OverID2:"+overObjID_2);
	...
	if (allID2.indexOf(overObjID_2)>=0)
	{
			//this.myAlert("冒泡:"+((allID2.indexOf(overObjID_2)>=0)||(allID2.indexOf((this.currentObjID).substring(0,2))>=0)));
			event.stopPropagation();    //阻止冒泡
			event.preventDefault();
...
SVG.prototype.onMouseUp=function(e,event){  //mouse up
	var startx = parseInt(e.pageX-$("#"+this.ID).offset().left);
	var starty = parseInt(e.pageY-$("#"+this.ID).offset().top);
	...
	if (this.mouseState=="down"){
	    ...
	    */
		...
		if ((allID2.indexOf(overObjID.substr(0,2))>0)&& (Math.abs(this.startPoint[0]-startx)<15) && (Math.abs(this.startPoint[1]-starty)<15))
		{
			$("#"+this.ID+"initLine").attr("x1",-1);//防止作点而移动在点背景范围内当成了作线段
				...
				if (this.smartTip.type!="")
				{
					...
					//		this.newCon("x"+this.newChildID(),cId,"X"+this.startPoint[2]);
						...
						case "EPE"://垂直相等
							...
							var y2=starty;
							var type=1;
							//this.myAlert("type:"+typeof x1+","+typeof y1+","+typeof x2+","+typeof y2);
							...
							//this.myAlert(p2_y+":"+czx1+","+czx2+","+y1);
					...
					switch (overObjID.substring(0,2)){
						...
						case "BC":
							...
							{					   
							   ...
							   startx=$("#"+e.target.id.substring(1)).attr("cx");//纠正在点上的偏离,是否直接取overOjb
							   starty=$("#"+e.target.id.substring(1)).attr("cy");
							   //alert("e.target.id:"+overObjID);
							   //clone线，修改ID，添加，还原initLine，添加限制条件
							   var lineID="PP"+this.newChildID();
							   /*var line=$("#"+this.ID+"initLine").clone();
							   ...
							   this.newCon("c"+this.newChildID(),"LPP,"+lineID+","+this.startPoint[2]+","+e.target.id.substring(1),"C"+this.startPoint[2]);
							   //this.newCon("C"+e.target.id,selfID,"LPP,"+lineID+","+this.startPoint[2]+","+e.target.id);
							   this.newCon(selfID,"LPP,"+lineID+","+this.startPoint[2]+","+e.target.id.substring(1),"C"+e.target.id.substring(1));
...
SVG.prototype.reDraw=function(width,height){  //重绘
	//alert("重绘");
...
SVG.prototype.addRow=function(id){
	...
	n.bind("keyup", function(e) {//keyup DOMNodeInserted DOMNodeRemoved ，DOMNodeInserted,DOMCharacterDataModified
		...
		/*othis.myAlert("ltxStr:"+ltxStr);
		if ((ltxStr.substring(0,1)).toUpperCase()=="P")//作点
		{
			var latex=ltxStr.substring(1).split(",");
		}*/
		...
		//othis.myAlert("iString:"+iString);
		switch (iString.substring(0,1))
		{
			case "P":
			case "p"://动态点
				if (iString.indexOf(",")>0)
				{
					...
					var iStr=iString.split(",");
					iStr[0]=iStr[0].substring(2);//去掉P(
					iStr[1]=iStr[1].substring(0,iStr[1].length-1);//去掉最后的)
					/*var st=new Array();
					var result=new Array();
					
					for (var i=0;i<2;i++ )
					{
						st[i]=mathConvert(iStr[i]);
						var def="",undef="";
						alert("st[i]:"+i+"|"+st[i]);
						result[i]=othis.checkExp(st[i],def,undef);
						//result[i][2]=filterRepeatStr(result[i][2]);
						//result[i][3]=filterRepeatStr(result[i][3]);
					...
					if (result[0][0]>0 && result[1][0]>0)
					{
						...
						alert(result[0][0] +":"+ result[1][0]);
					}*/
					var m1=mathConvert(iStr[0]);
					var m2=mathConvert(iStr[1]);
					var def="",undef="";
					var result1=othis.checkExp(m1[0],def,undef);				
					result1[2]=filterRepeatStr(result1[2]);
					result1[3]=filterRepeatStr(result1[3]);

					//othis.myAlert("m1[0]:"+iStr[0]+"|"+m1[0]+"|"+result1[0]);

					var result2=othis.checkExp(m2[0],def,undef);				
					result2[2]=filterRepeatStr(result2[2]);
					result2[3]=filterRepeatStr(result2[3]);

					othis.myAlert("m1[0]:"+iStr[1]+"|"+m2[0]+"|"+result2[0]+"|"+result2[1]+"|"+result2[3]);
					...
					{
						$("#"+"TIP"+ID).html("");   
						if ((result1[3]!=null && result1[3]!="") || (result2[3]!=null && result2[3]!="")) //未定义变量
						{					 
							othis.addVarTip(ID,iString,result1[3]+result2[3],result1[1],result2[1]); 
						}
						else{
							//增加动态点
							$("#"+"ADD"+ID).html("");
							if ($("CP"+ID).length>0)//该点已经存在
							{
								var x,y;
								eval("x = function(t){ with(Math) return "+othis.mathjs(result1[1])+" }");
								eval("y = function(t){ with(Math) return "+othis.mathjs(result2[1])+" }");
								var point=othis.u2s([x,y]);
								othis.movePoint("CP"+ID,point[0],point[1]);
							}
							else{
								var x,y;
								eval("x = function(t){ with(Math) return "+othis.mathjs(result1[1])+" }");
								eval("y = function(t){ with(Math) return "+othis.mathjs(result2[1])+" }");
								var point=othis.u2s([x,y]);
								var label=othis.newLabel();
								//function(id_point,startx,starty,label,t)
								othis.newPoint("CP"+ID,point[0],point[1],label);
								//添加约束条件


								if ((result1[2]!=null && result1[2]!="") || (result2[2]!=null && result2[2]!=""))//已定义变量
								{
								  //alert("result[2]:"+result[2]);
								  var m=(result1[2]+result2[2]).split("");
							...
							//newPoint=function(id_point,startx,starty,label,t)
						}//end if result[3]
						...
						if (result1[0]>0)
						{
							$("#"+"TIP"+ID).html(result2[1]);
						}
						else{
							$("#"+"TIP"+ID).html(result1[1]);
						}
...
SVG.prototype.addMouseEvent=function(){  //添加事件
   var othis=this;
    //zheng 2016.3.13
   $(document).on('tap click',"#"+this.ID,function(event){
	   //alert("othis,this:"+othis+"::"+this);
	   var e = event.type == 'touchend' ? event.originalEvent.changedTouches[0] : event;
	   othis.onMouseClick(e,event);
   });
   ...
   $(document).on('touchstart mousedown',"#"+this.ID,function(event){
		var e = event.type == 'touchstart' ? event.originalEvent.touches[0] : event;
	    othis.onMouseDown(e,event);
   });
   $(document).on('touchmove mousemove',"#"+this.ID,function(event){
		var e = event.type == 'touchmove' ? event.originalEvent.touches[0] : event;
	   othis.onMouseMove(e,event);
   });
   $(document).on('touchend mouseup',"#"+this.ID,function(event){
		var e = event.type == 'touchend' ? event.originalEvent.changedTouches[0] : event;
	   othis.onMouseUp(e,event);
   });
...
SVG.prototype.newDynPoint=function(id_point,startx,starty,label,t){//新生成动态点，t 决定是否更新指令数组
...
SVG.prototype.newAngle=function(id,p1,p2,p3,t){//新增加角标注
  ...
  if (x1==x2)
  {
   ...
   if (alpha<=180)
   {
	   ...
	   alpha=(360-alpha).toFixed(this.decimal);
   }
  ...
  //alpha=Math.acos( -((x2-x1)*(x3-x2)+(y2-y1)*(y3-y2))/Math.sqrt(((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1))*((y3-y2)*(y3-y2)+(x3-x2)*(x3-x2))))/Math.PI*180;
   ...
   if (d==0)
   {
      return;
	   ...
	   sy=((y1-y2)/d*r+y2).toFixed(this.decimal);  //这儿假设标注距离为15
	   sx=((x1-x2)/d*r+x2).toFixed(this.decimal);
   }
   d=Math.sqrt((y2-y3)*(y2-y3)+(x2-x3)*(x2-x3));
   if (d==0)
   {
      return;
	   ...
	   ey=((y3-y2)/d*r+y2).toFixed(this.decimal);  //这儿假设标注距离为15
	   ex=((x3-x2)/d*r+x2).toFixed(this.decimal);
   }
   if (alpha<=180)
   {
