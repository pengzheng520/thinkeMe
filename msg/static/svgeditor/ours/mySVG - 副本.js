	...
	for (var i=0;i<actA.length ;i++ )
	{
		...
		switch ($.trim(comm[0]))
		{
		    case "ChC"://changeColor
				changeColor(comm[1],comm[2],false);
				break;
			case "NP"://new Point			    
...
SVG.prototype.addMouseEvent=function(){  //添加事件
   ...
   $("#"+this.ID).addLeftEvent(this.ID);//左边水表事件
...
var ID=id+"I"
  ...
  $(".dcg-options-menu").find("span.dcg-color-option").click(function(e){
	  ...
	  $(".dcg-options-menu").hide();  
		...
		changeColor("fx"+_thisid.substring(3),_thiscolor.split("-")[1]);
...
function changeColor(id,color,t){
	if (typeof t == "undefined")
	{
