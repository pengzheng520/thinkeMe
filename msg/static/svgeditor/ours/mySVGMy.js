...
var allID2=" CP FP BC BF CR PP fx ";//用于解决firefox下阻止鼠标默认动作
...
function SVG(parentID,ID,width,height,xMin,xMax,yMin,yMax,gridUnit,border,xy,label,editable,sys){ //
	...
	this.xMin=(typeof xMin == "undefined")?-5:xMin;
	this.xMax=(typeof xMax == "undefined")?5:xMax;
	...
	if (this.editable)
	{
		this.width=(typeof width == "undefined" || width==null)?($("#"+parentID).width()*2/3):width;
	}
	else{
...
<div id="ParentID"> 
	<div id="SVG+ParentID+P" class="row">
		<div id="SVG+ParentID+L" class="col-xs-3">   <!--左边区域-->
		...
		<div id="SVG+ParentID+R" class ="col-xs-9">   <!--右边区域-->
			<div id="SVG+ParentID+tool">  <!--右边SVG工具栏toolBar区-->
			</div>
...
*/
   ...
   parentDiv.addClass('row'); 
	...
	if (this.editable)
	{
	   ...
	   var menu=$("<div></div>");  //交互菜单,left
	   menu.attr('id',this.ID+"M");     
	   menu.addClass('col-xs-3'); 
	   ...
	   var menu2=$("<div class=\"btn-group\" role=\"group\" aria-label=\"...\">"
						+"<button id="+this.ID+"Edit"+" type=\"button\" class=\"btn btn-primary\">编辑</button>"
						+"<button type=\"button\" class=\"btn btn-default\">===</button>"
						+"<button id="+this.ID+"Complete"+" type=\"button\" class=\"btn btn-primary\">完成</button>"
				  +"</div>");	
		menu.append(menu2);

		
		var interArea=$("<div></div>");  //交互对象区,left
		interArea.attr('id',this.ID+"I");
		menu.append(interArea);
		var m=$("<button id="+this.ID+"addRow"+" type=\"button\" class=\"btn btn-primary\">添加一行</button>");
		menu.append(m);

		parentDiv.append(menu);
	}
	else{//else if (this.editable)

	}//end if (this.editable)
   ...
   if (this.editable)
   {
	...
	var svg_edt_data = 
      ...
      +'<div class="btn-group" role="group" id="svgdrag">'
		  +'<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="width:40px"><span class="caret"></span></button>'
...
element.setAttributeNode(attr);*/
	...
	//alert(this.origin[0]+","+this.origin[1]+":"+this.gridUnit);
...
//设置了大小就有问题哈
   ...
   //this.height=(typeof h == "undefined")?Math.floor($(window).height()/this.gridUnit)*this.gridUnit:h;
	...
	if (this.editable)
	{
		this.width=(typeof width == "undefined" || width==null)?($("#"+(this.ID).substr(1)).width()*2/3):width;
	}
	else{
				  ...
				  if(iString.indexOf(",")>=0)//参数形式
				   {
					 // function(id,type,fun,x_min,x_max,points,min_type,max_type,stroke1,strokewidth1,strokedash1,pg,t) 
					  othis.plot("fx"+ID,"x=f(t)",result[1],-5,5,null,null,null,"black",2,null,1);
				   }
				  else if(iString.indexOf("x=")>=0)//x=f(y)形式
				   {
						  othis.plot("fx"+ID,"x=f(y)",result[1],-5,5,null,null,null,"black",2,null,1);
				   }
				  else if(iString.indexOf("r=")>=0)//极坐标形式
						  othis.plot("fx"+ID,"r=f(t)",result[1],-5,5,null,null,null,"black",2,null,1);
				  else								//y=f(x)形式
					...
					//othis.plot("fx"+ID,"y=f(x)","1- x",-5,5,null,"fx"+ID,null,null,"black",2,null,1);
						  othis.plot("fx"+ID,"y=f(x)",result[1],-10,10,null,null,null,"black",2,null,1);
				     //已定义变量的约束条件result[2]
    ...
    */
	...
	var leftdata =  '<div class="dropdown icon-list">'+
                     ...
                     '<ul class="dropdown-menu">'+
					   ...
					   '<li role="separator" class="divider"></li>'+
					   '<li class="line_open_close"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span></li>'+
					   '<li role="separator" class="divider"></li>'+
                       '<li><i title="改变颜色" class="fa fa-color color-C0504D"></i></li>'+
					   '<li role="separator" class="divider"></li>'+
                       '<li><i title="虚线实线" class="dcg-graph-icon dcg-graph-dashed"></i></li>'+
					 '</ul>'+
                    '</div>'	
			   ...
			   switch (con[0])
			   {
				   case "var"://函数
                       ...
                       if (funSrc!=null && funSrc!=undefined)
                       {
                           ...
                           //         0       1        2        3         4          4            6              7          8                  9
					       //plot=function(id,type,fun,x_min,x_max,points,min_type,max_type,stroke1,strokewidth1,strokedash1,pg)
						   ...
						   this.plot(con[1],para[0],para[1],para[2],para[3],null,para[4],para[5],para[7],para[8],para[9],1,false);
...
function addLeftEvent(id){
//记录需要用到变量
var ID=id+"I"
  var recordID = {
	  rank:"EXSSVGParent1001"                       //记录现在鼠标的焦点在第几个li标签上
  }   
