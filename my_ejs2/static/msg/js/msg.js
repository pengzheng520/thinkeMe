    ...
    msg.prototype = {
      ...
      EV_init:function(){
          ...
          self.socket.on('getUser',function(data){
              if(data) {
                  ...
                  var data = $.each(data,function(n,v){
                      v.id = v.userID;
      ...
      addEvent:function(){
          ...
          $("#thinkMsg").on('click','#addFriendBox button.j-search',function(e){
              ...
              if(valId == ""){
                  return false;
              }else if(valId == self.userName){
                  $("#addFriendBox").attr("class","m-dialog done");
                  $("#addFriendBox div.tip").text("别看了就是你自己");
              }else if($('#singleFriends li[data-userName="'+valId+'"]').length == 1){
                  $("#addFriendBox").attr("class","m-dialog friend");
                  var friendsLi = $('#singleFriends li[data-userName="'+valId+'"]');
                  ...
                  var this_id = friendsLi.attr('data-id');
                  //点击聊天
                  $("#addFriendBox button.j-chat").unbind().click(function(e){
                      self.showChatFace(this_id,$('#singleFriends li[data-id="'+this_id+'"]'));
                      $("#j-mask").hide();
                      $("#addFriendBox").hide().attr("class","m-dialog");
                  });
              }else{
          ...
          //删除好友
          $("#thinkMsg").on("click","#singleFriends .del-friends",function(e){
              var $li = $(this).parent();
              var id = $li.attr('data-id');
              var delFriends =   layer.confirm('是否删除此好友？', {
                                      btn: ['不,点错了！','确定'] //按钮
                                  }, function(){
                                      layer.close(delFriends);
                                  }, function(){
                                      showMack();
                                      self.delFriends(id,$li);
                                  });
              return false;
          })
          ...
          //白板和电子书切换按钮
          //白板
          $("#thinkMsg").on("click",".btn-whiteboard",function(e){
              var face =  $(this).parents('.group-interface .interface-bodyer-one');
              face.find(".whiteboard").show();
              face.find(".eBook").hide();
          });
          //电子书
          $("#thinkMsg").on("click",".btn-eBook",function(e){
              var face =  $(this).parents('.group-interface .interface-bodyer-one');
              face.find(".whiteboard").hide();
              face.find(".eBook").show();
          });

          //群成员隐藏,显示
          $("#thinkMsg").on("click",".group-members .header",function(e){
              var leftFace = $(this).parents(".interface-bodyer-one");
              var height = leftFace.height();
              var groupMembersHeight =  $(this).parents(".group-members").height();

              if(groupMembersHeight < 40) {
                  leftFace.find('.interface-content').height(height - 140);
                  $(this).parents(".group-members").height('140px');
              }else{
                  leftFace.find('.interface-content').height(height - 24);
                  $(this).parents(".group-members").height('24px');
              }
              return false;
          })
      ...
      //删除好友
      delFriends : function(id,$this){
          var self = this ;
          var data = {
              sender   : self.id,
              friends  : id
          };
          self.socket.emit('delFriends',data,function(data){
              hideMack();
              if(data.code == 200){
                    $this.remove();
              }
      ...
      addFriends : function(data){
          ...
          self.socket.emit('addFriends',data,function(msg){
              if(msg.code == 200){
                  msg.data[0].name = msg.data[0].userNickname
                  ...
                  $("#addFriendBox button.j-add").unbind().click(function(e){
                      self.socket.emit('saveFriends',{sender:self.id,addFriend:msg.data[0].userID},function(){
                          $("#addFriendBox").attr("class","m-dialog done");
                          $("#addFriendBox div.tip").text("添加好友成功！");
                          var friends = {
                              id        : msg.data[0].userID,
                              name      : msg.data[0].name,
                              img       : msg.data[0].img,
                              userName  : data.addFriend
                          };
                          //当没有这个用户,生成这个用户
                          var liFriend = self.appendFriendS([friends]);
                          $("#singleFriends").append(liFriend);
      ...
      latex_html : function(text){
          ...
          latex_html.find(".mathquill-embedded-latex").each(function(index, element) {
              var lat = $(this).html();
              var diframe = '<iframe class="mathquill-disbedded-latex" src="/msg/disformula.html" data-latex="'+lat+'" frameborder="0"></iframe>';
     ...
     onloadIframe:function($iframe,callback,callback1) {
      ...
      mathTool : function(){
          ...
          $("#thinkMsg").on('click',".edui-formula-latex-item",function(e){
              ...
              if(ismath == "true"){
                  ...
                  var zIframe = '<iframe class="mathquill-embedded-latex" src="/msg/formula.html" data-latex="'+dataLatex+'" frameborder="0"></iframe>';
                  if ($.browser.msie){
      ...
      zoomInterface : function(){
          ...
          $("#thinkMsg").on('click',".interface-max",function(e){
              ...
              var gMembers = interface.find(".group-members");
              if(gMembers.length == 1 && gMembers.height()<30){
                  interface.find('.interface-bodyer-one .interface-content').css('height',height-69);
              }
          ...
          $("#thinkMsg").on('click',".interface-maxmin",function(e){
              ...
              var gMembers = interface.find(".group-members");
              if(gMembers.length == 1 && gMembers.height()<30){
                  interface.find('.interface-bodyer-one .interface-content').css('height',376);
              }
        ...
        var data = '<div id="thinkMsg">'+
                        ...
                        '<div class="im_chat_window" id="imChatWindow">'+
                            ...
                            '<div class="im_chat_body" id="imChatBody">'+
                                '<div class="friends">'+
                                    '<div id="addFriends" class="create-group">添加好友</div>'+
      ...
      appendFriendS : function(friends){
       if(friends != null) {
           ...
           $.each(friends, function (n, value) {
               data += '<li class="panel_item" data-id="'+value.id+'" data-userName="'+value.userName+'">' +
                           '<div class="panel_avatar"><img class="panel_image" src="' + value.img + '" /></div>' +
                           '<span class="badge">0</span>'+
                           '<div class="panel_text"><p class="panel_single-row">' + value.name + ' ('+value.userName+')</p></div>' +
                           '<i class="icon iconfont del-friends" title="删除好友">&#xe600;</i>'+
                       '</li>';
           });
           return data;
       }else{
      ...
      getGroupInterface : function(groupData){
        ...
        var data = '<div class="interface group-interface" data-createUser="'+groupData.cleateUser+'" data-id="'+groupData.classID+'" data-groupID="'+groupData.classdomian+'" style="left:'+left+';top:'+top+'">'+
                        '<div class="interface-header">'+
                            '<img class="user-photo" src="/image/group.jpg" />'+
                            ...
                            '</span>'+
                        '</div>'+
                        '<div class="interface-bodyer interface-bodyer-one  btn-group">'+
                            '<button type="button" class="btn btn-whiteboard">白板</button>'+
                            '<div class="interface-content whiteboard">' +
                                '<iframe  src="/svgeditor/index.html" frameborder="0" width="100%" height="100%"></iframe>'+
                            '</div>'+
                            '<div class="interface-content eBook">'+
                                '<iframe  src="/ebook/ebook.html" frameborder="0" width="100%" height="100%"></iframe>'+
                            '</div>'+
                            '<div class="group-members">'+
                             '<div class="header"><span>群成员</span></div>'+
                             '<ul class="bodyer">'+self.newClassUser(groupData.user)+'</ul>'+
                            '</div>'+
                            '<button type="button" class="btn btn-eBook">电子书</button>'+
                        '</div>'+
                        '<div class="interface-bodyer">'+
                            '<div class="interface-content chat-content">'+
                                '<p class="u-msgTime">- - - - - '+msg._getTimeStamp()+' - - - - -</p>'+
      ...
      addClassModal:function(){
        ...
        newClass : function(value){
              var data = '<li class="panel_group_item" data-id="'+value.classID+'" data-pwd="'+value.classPWD+'">'+
                            '<div class="panel_avatar"><img class="panel_image" src="/image/group.jpg" /></div>'+
...
//显示隐藏遮罩
function showMack() {
    window.baseMack = layer.load(1, {
        shade: [0.5, '#000'] //0.1透明度的白色背景
    });
}
function hideMack() {
    layer.close(window.baseMack);
}
