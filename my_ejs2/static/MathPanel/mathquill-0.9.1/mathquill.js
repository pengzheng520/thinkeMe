...
LatexCmds['√'] = P(MathCommand, function(_, _super) {
  ...
  _.redraw = function() {
    ...
    if(block.is(":hidden")){
       var a = $('<span class="mathquill-rendered-math">'+block.html()+'</span>').appendTo("body");
       var h = a.innerHeight() - 10;
       a.remove();
    }else{
      h = block.innerHeight();
    }
    scale(block.prev(), 1, h/+block.css('fontSize').slice(0,-2) - .1);
  };
});
